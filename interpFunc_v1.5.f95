...
SUBROUTINE MLPG_GET_ETA(FSDOM, NFS, XFS, YFS, ZFS, NOT, XOT, YOT, ZOT, ERR, DDL, NLMAX, PRINTERRMSG)
   ...
   REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),AA(:,:,:)
   REAL(KIND=8)::A(4,4),AINV(4,4),PT(4)
   REAL(KIND=8)::PB2(4),PP2(4,4)
   REAL(KIND=8)::XQ,YQ,ZQ,PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI

   ALLOCATE(B(4,NLMAX,NOT),NNN(NOT),PHII(NLMAX,NOT),AA(4,NLMAX,NOT))
   ...
   !$acc& NFS,XFS,YFS,NLMAX,RIAV,PLANEID,ERR,PRINTERRMSG,B,AA,NNN)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(4) private(ND,W,PHI,A,PB2,PP2,PT,AINV)
   DO INOD=1,NOT
      ...
      CALL SHAPEFUN_PHI_SHA(4,NLMAX,NN,AINV,B(:,:,INOD),PT,AA(:,:,INOD),PHI,I,J,K)
