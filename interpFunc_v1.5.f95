...
SUBROUTINE MLPG_GET_ETA(FSDOM, NFS, XFS, YFS, ZFS, NOT, XOT, YOT, ZOT, ERR, DDL, NLMAX, PRINTERRMSG)
   ...
   REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),AA(:,:,:),PHI(:,:),W(:,:)
   REAL(KIND=8)::A(4,4),AINV(4,4),PT(4)
   REAL(KIND=8)::PB2(4),PP2(4,4)
   REAL(KIND=8)::XQ,YQ,ZQ,PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI

   ALLOCATE(B(4,NLMAX,NOT),NNN(NOT),PHI(NLMAX,NOT),AA(4,NLMAX,NOT),W(NLMAX,NOT))
   ...
   !$acc& NFS,XFS,YFS,NLMAX,RIAV,PLANEID,ERR,PRINTERRMSG,B,AA,PHI,W,NNN)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(8) private(ND,A,PB2,PP2,PT,AINV)
   DO INOD=1,NOT
      ...
      CALL SHAPPARA_R_2D_SHA(NFS,4,NLMAX,A,B(:,:,INOD),NN,ND,W(:,INOD),&
