...
SUBROUTINE MLPG_GET_UP(DOMIN,NIN,XIN,YIN,ZIN,UIN,VIN,WIN,PIN,&
   ...
   INTEGER(KIND=4)::I,J,K,I2,NI
   INTEGER(KIND=4)::IX,IY,IZ,IPOS,IX1,IY1,IZ1
   REAL(KIND=8),MANAGED,ALLOCATABLE::PHI(:,:),B(:,:,:),AA(:,:,:),W(:,:),PP2(:,:,:)
   REAL(KIND=8)::A(4,4),AINV(4,4),PT(4)
   REAL(KIND=8)::PB2(4)
   REAL(KIND=8)::XQ,YQ,ZQ,PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI
   ...
   ALLOCATE(NNN(NOT))
   ALLOCATE(PHI(NLMAX,NOT),B(4,NLMAX,NOT),AA(4,NLMAX,NOT),W(NLMAX,NOT),PP2(4,4,NOT))

   RIAV=2.1D0*DDL

   !$acc data copyin(NLMAX,RIAV,NIN,&
   !$acc&  XIN,YIN,ZIN,UIN,VIN,WIN,PIN,NOT,XOT,YOT,ZOT,UOT,&
   !$acc&  VOT,WOT,POT,DOMIN,PHI,B,AA,W,PP2)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(16) private(ND,A,PB2,PT,AINV)
   DO INOD=1,NOT
      ...
      CALL SHAPEFUN_PHI_SHA(4,NLMAX,NN,AINV,B(:,:,INOD),PT,AA(:,:,INOD),PHI(:,INOD),I,J,K)
