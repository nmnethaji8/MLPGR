...
ETMP=DEXP(-(1D0**2))
 ...
 !! Z=0 FOR FREE-SURFACE DOMAIN
DR=0
CALL FINDCELL2(FSDOM,XQ,YQ,ZQ,IX,IY,IZ,IPOS)
...
SUBROUTINE MLPG_GET_ETA(FSDOM, NFS, XFS, YFS, ZFS, NOT, XOT, YOT, ZOT, ERR, DDL, NLMAX, PRINTERRMSG)
   ...
   !$acc& NFS,XFS,YFS,NLMAX,RIAV,PLANEID,ERR,PRINTERRMSG)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(2) private(ND,W,PHI,A,B,PB2,PP2,PT,AINV,B,AA)
   DO INOD=1,NOT
...
SUBROUTINE MLPG_GET_UP(DOMIN,NIN,XIN,YIN,ZIN,UIN,VIN,WIN,PIN,&
  ...
  ALLOCATE(PHII(NLMAX,NOT),NNN(NOT),BB(4,NLMAX,NOT))!AAA(4,4,NOT)

  RIAV=2.1D0*DDL

  !$acc data copyin(NLMAX,RIAV,NIN,&
  !$acc&  XIN,YIN,ZIN,UIN,VIN,WIN,PIN,NOT,XOT,YOT,ZOT,UOT,&
  !$acc&  VOT,WOT,POT,DOMIN,NNN,BB)
