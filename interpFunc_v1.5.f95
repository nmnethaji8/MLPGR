...
SUBROUTINE MLPG_GET_UP(DOMIN,NIN,XIN,YIN,ZIN,UIN,VIN,WIN,PIN,&
   ...
   REAL(KIND=8),MANAGED,ALLOCATABLE::PHI(:,:),W(:,:),A(:,:,:)
   REAL(KIND=8)::AINV(4,4),PT(4)
   REAL(KIND=8)::PB2(4),PP2(4,4)
   REAL(KIND=8)::XQ,YQ,ZQ,AA(4,NLMAX),PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI
   REAL(KIND=8)::PTMP,UTMP,VTMP,WTMP
   ...
   ALLOCATE(PHI(NLMAX,NOT),W(NLMAX,NOT),A(4,4,NOT),NNN(NOT))

   RIAV=2.1D0*DDL

   !$acc data copyin(NLMAX,RIAV,NIN,&
   !$acc&  XIN,YIN,ZIN,UIN,VIN,WIN,PIN,NOT,XOT,YOT,ZOT,UOT,&
   !$acc&  VOT,WOT,POT,DOMIN,PHI,W,A)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(32) private(ND,AA,B,PB2,PP2,AINV,PT)
   DO INOD=1,NOT
      ...
      CALL SHAPEFUN_PHI_SHA(4,NLMAX,NN,AINV,B,PT,AA,PHI(:,INOD),I,J,K)
