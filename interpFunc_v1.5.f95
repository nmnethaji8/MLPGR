...
SUBROUTINE MLPG_GET_UP(DOMIN,NIN,XIN,YIN,ZIN,UIN,VIN,WIN,PIN,&
   ...
   INTEGER(KIND=4)::I,J,K,I2,NI
   INTEGER(KIND=4)::IX,IY,IZ,IPOS,IX1,IY1,IZ1
   REAL(KIND=8)::B(4,NLMAX)
   REAL(KIND=8),MANAGED,ALLOCATABLE::PHI(:,:),W(:,:),AINV(:,:,:),A(:,:,:),PT(:,:)
   REAL(KIND=8)::PB2(4),PP2(4,4)
   REAL(KIND=8)::XQ,YQ,ZQ,AA(4,NLMAX),PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI
   REAL(KIND=8)::PTMP,UTMP,VTMP,WTMP
   ...
   ALLOCATE(PHI(NLMAX,NOT),W(NLMAX,NOT),AINV(4,4,NOT),A(4,4,NOT),PT(4,NOT),NNN(NOT))

   RIAV=2.1D0*DDL

   !$acc data copyin(NLMAX,RIAV,NIN,&
   !$acc&  XIN,YIN,ZIN,UIN,VIN,WIN,PIN,NOT,XOT,YOT,ZOT,UOT,&
   !$acc&  VOT,WOT,POT,DOMIN,PHI,W,AINV,A,PT)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(32) private(ND,AA,B,PB2,PP2)
   DO INOD=1,NOT
      ...
      CALL SHAPPARA_R_SHA(NIN,4,NLMAX,A(:,:,INOD),B,NN,ND,W(:,INOD),&
         XIN,YIN,ZIN,I,NI,J,K,PB2,PP2,WWI)
      ...
      CALL SHAPEFUN_PHI_SHA(4,NLMAX,NN,AINV(:,:,INOD),B,PT(:,INOD),AA,PHI(:,INOD),I,J,K)
