...
SUBROUTINE MLPG_GET_UP(DOMIN,NIN,XIN,YIN,ZIN,UIN,VIN,WIN,PIN,&
   ...
   INTEGER(KIND=4)::I,J,K,I2,NI
   INTEGER(KIND=4)::IX,IY,IZ,IPOS,IX1,IY1,IZ1
   REAL(KIND=8),MANAGED,ALLOCATABLE::PHI(:,:),B(:,:,:),AA(:,:,:),W(:,:),PP2(:,:,:),AINV(:,:,:),A(:,:,:),PT(:,:),PB2(:,:)
   INTEGER(KIND=4),MANAGED,ALLOCATABLE::ND(:,:)
   REAL(KIND=8)::XQ,YQ,ZQ,PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI
   REAL(KIND=8)::PTMP,UTMP,VTMP,WTMP
   ...
   ALLOCATE(PHI(NLMAX,NOT),B(4,NLMAX,NOT),AA(4,NLMAX,NOT),W(NLMAX,NOT),PP2(4,4,NOT),AINV(4,4,NOT),A(4,4,NOT),ND(0:NLMAX,NOT),PT(4,NOT),PB2(4,NOT))

   RIAV=2.1D0*DDL

   !$acc data copyin(NLMAX,RIAV,NIN,&
   !$acc&  XIN,YIN,ZIN,UIN,VIN,WIN,PIN,NOT,XOT,YOT,ZOT,UOT,&
   !$acc&  VOT,WOT,POT,DOMIN,PHI,B,AA,W,PP2,AINV,A,ND,PT,PB2)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(32)
   DO INOD=1,NOT
      ...
      CALL SHAPPARA_R_SHA(NIN,4,NLMAX,A(:,:,INOD),B(:,:,INOD),NN,ND(:,INOD),W(:,INOD),&
      ...
      CALL SHAPEFUN_PHI_SHA(4,NLMAX,NN,AINV(:,:,INOD),B(:,:,INOD),PT(:,INOD),AA(:,:,INOD),PHI(:,INOD),I,J,K)
...
30    UTMP=0D0
      ...
      DO I2=1,NN
         UTMP=UTMP+PHI(I2,INOD)*UIN(ND(I2,INOD))
         VTMP=VTMP+PHI(I2,INOD)*VIN(ND(I2,INOD))
         WTMP=WTMP+PHI(I2,INOD)*WIN(ND(I2,INOD))
         PTMP=PTMP+PHI(I2,INOD)*PIN(ND(I2,INOD))
      ENDDO
      UOT(INOD)=UTMP
      VOT(INOD)=VTMP
      WOT(INOD)=WTMP
