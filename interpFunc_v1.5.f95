...
SUBROUTINE MLPG_GET_ETA(FSDOM, NFS, XFS, YFS, ZFS, NOT, XOT, YOT, ZOT, ERR, DDL, NLMAX, PRINTERRMSG)
   ...
   REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),AA(:,:,:),PHI(:,:),W(:,:),AINV(:,:,:),A(:,:,:)
   REAL(KIND=8)::PT(4)
   REAL(KIND=8)::PB2(4),PP2(4,4)
   REAL(KIND=8)::XQ,YQ,ZQ,PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI

   ALLOCATE(B(4,NLMAX,NOT),NNN(NOT),PHI(NLMAX,NOT),AA(4,NLMAX,NOT),W(NLMAX,NOT),ND(0:NLMAX,NOT),AINV(4,4,NOT),A(4,4,NOT))
   ...
   !$acc& NFS,XFS,YFS,NLMAX,RIAV,PLANEID,ERR,PRINTERRMSG,B,AA,PHI,W,NNN,ND,AINV,A)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(16) private(PB2,PP2,PT)
   DO INOD=1,NOT
