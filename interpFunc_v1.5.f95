...
SUBROUTINE MLPG_GET_UP(DOMIN,NIN,XIN,YIN,ZIN,UIN,VIN,WIN,PIN,&
   ...
   REAL(KIND=8),MANAGED,ALLOCATABLE::PHI(:,:),W(:,:)
   REAL(KIND=8)::A(4,4),AINV(4,4),PT(4)
   REAL(KIND=8)::PB2(4),PP2(4,4)
   REAL(KIND=8)::XQ,YQ,ZQ,AA(4,NLMAX),PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI
   ...
   ALLOCATE(PHI(NLMAX,NOT),W(NLMAX,NOT),NNN(NOT))

   RIAV=2.1D0*DDL

   !$acc data copyin(NLMAX,RIAV,NIN,&
   !$acc&  XIN,YIN,ZIN,UIN,VIN,WIN,PIN,NOT,XOT,YOT,ZOT,UOT,&
   !$acc&  VOT,WOT,POT,DOMIN,PHI,W)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(32) private(ND,AA,B,PB2,PP2,AINV,PT,A)
   DO INOD=1,NOT
      ...
      CALL SHAPPARA_R_SHA(NIN,4,NLMAX,A,B,NN,ND,W(:,INOD),&
         XIN,YIN,ZIN,I,NI,J,K,PB2,PP2,WWI)
