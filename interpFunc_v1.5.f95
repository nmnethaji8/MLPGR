...
SUBROUTINE MLPG_GET_ETA(FSDOM, NFS, XFS, YFS, ZFS, NOT, XOT, YOT, ZOT, ERR, DDL, NLMAX, PRINTERRMSG)
   ...
   REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),AA(:,:,:),PHI(:,:),W(:,:),AINV(:,:,:),A(:,:,:),PP2(:,:,:)
   REAL(KIND=8)::PT(4)
   REAL(KIND=8)::PB2(4)
   REAL(KIND=8)::XQ,YQ,ZQ,PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI

   ALLOCATE(B(4,NLMAX,NOT),NNN(NOT),PHI(NLMAX,NOT),AA(4,NLMAX,NOT),W(NLMAX,NOT),ND(0:NLMAX,NOT),AINV(4,4,NOT),A(4,4,NOT),PP2(4,4,NOT))
   ...
   !$acc& NFS,XFS,YFS,NLMAX,RIAV,PLANEID,ERR,&
   !$acc& PRINTERRMSG,B,AA,PHI,W,NNN,ND,AINV,A,PP2)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(32) private(PB2,PT)
   DO INOD=1,NOT
      ...
      CALL WEIGHTF3_XY_SHA(FSDOM,NLMAX,NFS,XFS,YFS,XQ,YQ,&
         NN,ND(:,INOD),W(:,INOD),1,I,I2,IX,IY,IZ,IPOS,IX1,IY1,IZ1,RIAV,&
         ETMP,DR,DR2,WWI)
      ...
      CALL SHAPPARA_R_2D_SHA(NFS,4,NLMAX,A(:,:,INOD),B(:,:,INOD),NN,ND(:,INOD),W(:,INOD),&
         XFS,YFS,PLANEID,I,NI,J,K,PB2,PP2(:,:,INOD),WWI,ETMP)
      ...
      CALL SHAPEFUN_PHI_SHA(4,NLMAX,NN,AINV(:,:,INOD),B(:,:,INOD),PT,AA(:,:,INOD),PHI(:,INOD),I,J,K)

30    WWI=0D0
      !$acc loop seq
      DO I2=1,NN
         WWI=WWI+PHI(I2,INOD)*ZFS(ND(I2,INOD))
      ENDDO
      ZOT(INOD)=WWI
...
SUBROUTINE MLPG_GET_UP(DOMIN,NIN,XIN,YIN,ZIN,UIN,VIN,WIN,PIN,&
   ...
   INTEGER(KIND=4)::I,J,K,I2,NI
   INTEGER(KIND=4)::IX,IY,IZ,IPOS,IX1,IY1,IZ1
   REAL(KIND=8),MANAGED,ALLOCATABLE::PHI(:,:),B(:,:,:),W(:,:),PP2(:,:,:),AINV(:,:,:),A(:,:,:),PT(:,:),PB2(:,:)
   REAL(KIND=8)::XQ,YQ,ZQ,AA(4,NLMAX),PINT
   REAL(KIND=8)::RIAV,DR,DR2,ETMP,WWI
   REAL(KIND=8)::PTMP,UTMP,VTMP,WTMP
   ...
   ALLOCATE(PHI(NLMAX,NOT),B(4,NLMAX,NOT),W(NLMAX,NOT),PP2(4,4,NOT),AINV(4,4,NOT),A(4,4,NOT),PT(4,NOT),PB2(4,NOT),NNN(NOT))

   RIAV=2.1D0*DDL

   !$acc data copyin(NLMAX,RIAV,NIN,&
   !$acc&  XIN,YIN,ZIN,UIN,VIN,WIN,PIN,NOT,XOT,YOT,ZOT,UOT,&
   !$acc&  VOT,WOT,POT,DOMIN,PHI,B,W,PP2,AINV,A,PT,PB2)
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(32) private(ND,AA)
   DO INOD=1,NOT
      ...
      CALL WEIGHTF3_SHA(DOMIN,NLMAX,NIN,XIN,YIN,ZIN,XQ,YQ,ZQ,&
         NN,ND,W(:,INOD),1,I,I2,IX,IY,IZ,IPOS,IX1,IY1,IZ1,RIAV,&
         ETMP,DR,DR2,WWI)
      ...
      CALL SHAPPARA_R_SHA(NIN,4,NLMAX,A(:,:,INOD),B(:,:,INOD),NN,ND,W(:,INOD),&
         XIN,YIN,ZIN,I,NI,J,K,PB2(:,INOD),PP2(:,:,INOD),WWI)

      CALL BASEFUN_SHA(4,PT(:,INOD),XQ,YQ,ZQ)
      ...
      CALL SHAPEFUN_PHI_SHA(4,NLMAX,NN,AINV(:,:,INOD),B(:,:,INOD),PT(:,INOD),AA,PHI(:,INOD),I,J,K)
...
30    UTMP=0D0
      ...
      DO I2=1,NN
         UTMP=UTMP+PHI(I2,INOD)*UIN(ND(I2))
         VTMP=VTMP+PHI(I2,INOD)*VIN(ND(I2))
         WTMP=WTMP+PHI(I2,INOD)*WIN(ND(I2))
         PTMP=PTMP+PHI(I2,INOD)*PIN(ND(I2))
      ENDDO
      UOT(INOD)=UTMP
      VOT(INOD)=VTMP
      WOT(INOD)=WTMP
...
SUBROUTINE MLPG_GET_UP2(DOMIN, LNODE, NODEID, NWALLID, NIN, &
  ...
  REAL(KIND=8)::PTMP,UTMP,VTMP,WTMP
   ...
   !$acc parallel loop gang num_gangs(NOT) vector vector_length(128) private(ND,W,PHI,A,B,PB2,PP2,PT,A,AINV,PT,AA)
   DO INOD=1,NOT
