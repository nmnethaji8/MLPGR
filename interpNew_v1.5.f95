...
IMPLICIT NONE
  ...
  REAL(KIND=8)::PHI(NLMAX),B(MBA,NLMAX)
  REAL(KIND=8),MANAGED,ALLOCATABLE::W(:,:)
  ...
  !$acc& W,PT)
  !$acc parallel loop gang num_gangs(NODEID(-2)) vector vector_length(16) private(ND,ND2,AA,AINV,UINT,VINT,WINT,B,A,PP2,PB2,PHI)
  DO INOD=1,NODEID(-2)
   ...
   DO INTI=1,6
      ...
      IF(NN.LE.3)THEN
         CALL SHEPARDSF_SHA(PHI,NN,W(:,INOD),NLMAX,I,WWI)        
         GOTO 30
       ENDIF
      ...
      IF(MBA.EQ.4)THEN
       ...
       CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV,B,PT(:,INOD),AA,PHI,I,J,K)

      30    CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND,NN2,ND2,PHI,1/6D0)
   ...
   IF (NN.LE.0) THEN
    ...
    IF(MBA.EQ.4)THEN
      ...
      IF(ABS(WWI).LT.1E-15)THEN
        PRINT*,"     [ERR] SINGULAR MATRIX A, ADET ",WWI
        PRINT*,"     [---] LOC ",XQ,YQ,ZQ
        CALL SHEPARDSF_SHA(PHI,NN,W(:,INOD),NN,I,WWI)
        GOTO 31
      ENDIF
    ELSE
      PRINT*," [ERR] FINDINV NOT CODED FOR MBA =",MBA
    ...
    31  CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND,NN2,ND2,PHI,-1D0)
