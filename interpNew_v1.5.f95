...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::INOD,NN,IDWEI,KW2
   INTEGER(KIND=4)::I,J,K,I2,NI
   REAL(KIND=8),MANAGED,ALLOCATABLE::AA(:,:,:),AINV(:,:,:),PP2(:,:,:)
   REAL(KIND=8)::PHI(NLMAX),W(NLMAX),B(MBA,NLMAX)
   REAL(KIND=8)::A(MBA,MBA)
   REAL(KIND=8)::PT(MBA)
   REAL(KIND=8)::PB2(MBA)
   ...
   ALLOCATE(AA(MBA,NLMAX,NODEID(-1)),AINV(MBA,MBA,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)),ND(0:NLMAX,NODEID(-1)))
   ...
   !$acc& NLMAX,NODN,MBA,IDWEI,B,AA,AINV,PP2,ND,DDR)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16) private(A,B,W,PHI,PB2,PT)
   DO INOD=1,NODEID(-1)
      ...
      IF (NN.LE.0) THEN
       ...
       CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A,B,NN,ND(:,INOD),W,&
       COORX,COORY,COORZ,I,NI,J,K,PB2,PP2(:,:,INOD),WWI)

       CALL BASEFUN_SHA(MBA,PT,XQ,YQ,ZQ)

       IF(MBA.EQ.4)THEN
         CALL FINDINV4X4(A,AINV(:,:,INOD),WWI)
         IF(ABS(WWI).LT.1E-15)THEN
            PRINT*,"     [ERR] SINGULAR MATRIX A, ADET ",WWI
            PRINT*,"     [---] LOC ",XQ,YQ,ZQ
            CALL SHEPARDSF_SHA(PHI,NN,W,NLMAX,I,WWI)
      ...
      CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV(:,:,INOD),B,PT,AA(:,:,INOD),PHI,I,J,K)

      30  PINT=0D0
      !$acc loop reduction(+:PINT)
    DO I2=1,NN
      PINT=PINT+PHI(I2)*PTMP(ND(I2,INOD))      
    ENDDO
    P(INOD)=PINT
