...
SUBROUTINE VELOCITY_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
  ...
  REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),PHI(:,:),AA(:,:,:),W(:,:),PP2(:,:,:),A(:,:,:)
  REAL(KIND=8)::AINV(MBA,MBA),PT(MBA)
  REAL(KIND=8)::PB2(MBA)
  ...
  REAL(KIND=8)::UXL1,UYL1,UZL1,UXL3,UYL3,UZL3
   ...
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)),W(NLMAX,NODEID(-1)),ND(0:NLMAX,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)),A(MBA,MBA,NODEID(-1)))
  ...
  !$acc& UX,UY,UZ,UXC1,UYC1,UZC1,UXC3,UYC3,UZC3,GAMMA,GAMMA2,B,PHI,AA,W,ND,PP2,A)
  !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16) private(PB2,PT,AINV)
  DO INOD=1,NODEID(-1) 
   ...
   IF (NN.LE.0) THEN
    ...
    CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A(:,:,INOD),B(:,:,INOD),NN,ND(:,INOD),W(:,INOD),&
      ...
      IF(MBA.EQ.4)THEN
         CALL FINDINV4X4(A(:,:,INOD),AINV,WWI)
         IF(ABS(WWI).LT.1E-15)THEN
            PRINT*,"     [ERR] SINGULAR MATRIX A, ADET ",WWI,NODEID(INOD),NN
            PRINT*,"     [---] LOC ",XQ,YQ,ZQ
            CALL SHEPARDSF_SHA(PHI(:,INOD),NN,W(:,INOD),NLMAX,I,WWI)
