...
IMPLICIT NONE
  ...
  INTEGER(KIND=4)::INOD,ND(0:NLMAX),NN,NN2,KW2
  INTEGER(KIND=4),MANAGED,ALLOCATABLE::ND2(:,:)
  ...
  !$acc& PI,DT,UX,UY,UZ,LINKTAB,SKK,IVV,FB,AA,B,WORK,ND2,W)
  !$acc parallel loop gang num_gangs(NODEID(-2)) vector vector_length(4) private(UINT,&
  !$acc& VINT,WINT,ND,PHI,A,PB2,PP2,PT,AINV)
  DO INOD=1,NODEID(-2)
   ...
   DO INTI=1,6
      ...
      30    CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND,NN2,ND2(:,INOD),PHI,1/6D0)
   ...
   IF (NN.LE.0) THEN
    ...
    31  CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND,NN2,ND2(:,INOD),PHI,-1D0)
