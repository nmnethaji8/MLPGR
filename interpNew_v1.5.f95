...
IMPLICIT NONE
  ...
  REAL(KIND=8),MANAGED,ALLOCATABLE::A(:,:,:),PP2(:,:,:),PB2(:,:),PT(:,:)
  REAL(KIND=8)::AINV(MBA,MBA)
  ...
  ALLOCATE(B(MBA,NLMAX,NODEID(-2)),WORK(4*NLMAX,NODEID(-2)),W(NLMAX,NODEID(-2)))
  ALLOCATE(PHI(NLMAX,NODEID(-2)),A(MBA,MBA,NODEID(-2)),PP2(MBA,MBA,NODEID(-2)))
  ...
  !$acc& PI,DT,UX,UY,UZ,LINKTAB,SKK,IVV,FB,B,WORK,&
  !$acc& W,PHI,A,PP2,UINT,VINT,WINT,PB2,PT)
  !$acc parallel loop gang num_gangs(NODEID(-2)) vector vector_length(16) private(ND,ND2,AA,AINV)
  DO INOD=1,NODEID(-2)
   ...
   DO INTI=1,6
      ...
      IF(MBA.EQ.4)THEN
       ...
       CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV,B(:,:,INOD),PT(:,INOD),AA,PHI(:,INOD),I,J,K)
