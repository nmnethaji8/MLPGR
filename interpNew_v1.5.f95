...
SUBROUTINE VELOCITY_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
  ...
  REAL(KIND=8),MANAGED,ALLOCATABLE::PHI(:,:),AA(:,:,:),PP2(:,:,:),A(:,:,:),AINV(:,:,:),PT(:,:),PB2(:,:)
  ...
  REAL(KIND=8)::UXL1,UYL1,UZL1,UXL3,UYL3,UZL3
   ...
   ALLOCATE(PHI(NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)))
   ALLOCATE(ND(0:NLMAX,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)))
  ...
  !$acc& PHI,AA,ND,PP2,A,AINV,PB2,PT)
  !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16) private(B,W)
  DO INOD=1,NODEID(-1) 
   ...
   IF (NN.LE.0) THEN
    ...
    CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A(:,:,INOD),B,NN,ND(:,INOD),W,&
      ...
      CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV(:,:,INOD),B,PT(:,INOD),AA(:,:,INOD),PHI(:,INOD),I,J,K)
...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   !$acc& NLMAX,NODN,MBA,IDWEI,DDR)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16) private(ILINK,PP2,ND,AA,AINV,A,B,W,PHI,PB2,PT)
   DO INOD=1,NODEID(-1)
