...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::I,J,K,I2,NI
   REAL(KIND=8)::W(NLMAX)
   REAL(KIND=8),ALLOCATABLE::B(:,:,:),AA(:,:,:),PHI(:,:)
   REAL(KIND=8)::A(MBA,MBA),AINV(MBA,MBA),PT(MBA)
   REAL(KIND=8)::PB2(MBA),PP2(MBA,MBA)
   ...
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)))
   NNN=0
   ...
   !$acc& R,R0,CC,COORX,COORY,COORZ,NLINK,LNODE,NLMAX,NODN,MBA,IDWEI,B,AA,PHI)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(8) private(ND,&
   !$acc& W,A,PB2,PP2,PT,AINV)
   DO INOD=1,NODEID(-1)
