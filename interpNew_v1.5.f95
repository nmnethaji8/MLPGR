...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)),A(MBA,MBA,NODEID(-1)),AINV(MBA,MBA,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)),W(NLMAX,NODEID(-1)))
   NNN=0
   ...
   !$acc& R,R0,CC,COORX,COORY,COORZ,NLINK,LNODE,NLMAX,NODN,MBA,IDWEI,B,AA,PHI,A,AINV,PP2,W)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(32) private(ND,&
   !$acc& PB2,PT)
   DO INOD=1,NODEID(-1)
      ...
      IF (NN.LE.0) THEN
       ...
       CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A(:,:,INOD),B(:,:,INOD),NN,ND,W(:,INOD),&
      ...
      30  PINT=0D0
    DO I2=1,NN
      PINT=PINT+PHI(I2,INOD)*PTMP(ND(I2))      
    ENDDO
    P(INOD)=PINT
