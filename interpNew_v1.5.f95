   ...
   IMPLICIT NONE
     ...
     IF(MBA.EQ.4)THEN
      ...
      ELSEIF(MBA.EQ.9)THEN
         PRINT*,'[ERR] NOT CODED FOR BASEFNC QUAD'
         STOP
     ENDIF
...
!!----------------------- STIFFNESS_T1_SHA ------------------------!!
SUBROUTINE STIFFNESS_T1_SHA(NLMAX,NLMAX2,WORK,NN,ND,NN2,ND2,PHI,COEF)
!$acc routine seq
   IMPLICIT NONE
...
IMPLICIT NONE
  ...
  INTEGER(KIND=4)::INOD,NN,NN2,KW2
  INTEGER(KIND=4),MANAGED,ALLOCATABLE::ND2(:,:),ND(:,:)
  INTEGER(KIND=4)::I,J,K,I2,INTI,NI
  REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),W(:,:),PHI(:,:)
  REAL(KIND=8)::XQ,YQ,ZQ
  REAL(KIND=8),MANAGED,ALLOCATABLE::AA(:,:,:),A(:,:,:),AINV(:,:,:),PP2(:,:,:),PB2(:,:),PT(:,:)
  REAL(KIND=8)::RIAV,DR,DR2,R0I,TMPR,ROUI,WWI
  REAL(KIND=8)::PI,XINT,YINT,ZINT,WSUM
  REAL(KIND=8),MANAGED,ALLOCATABLE::WORK(:,:),UINT(:,:),VINT(:,:),WINT(:,:)
  REAL(KIND=8),MANAGED::DRINT(6,3)

  ALLOCATE(AA(MBA,NLMAX,NODEID(-2)),B(MBA,NLMAX,NODEID(-2)),WORK(4*NLMAX,NODEID(-2)),ND2(0:4*NLMAX,NODEID(-2)),W(NLMAX,NODEID(-2)))
  ALLOCATE(ND(0:NLMAX,NODEID(-2)),PHI(NLMAX,NODEID(-2)),A(MBA,MBA,NODEID(-2)),AINV(MBA,MBA,NODEID(-2)),PP2(MBA,MBA,NODEID(-2)))
  ALLOCATE(UINT(6,NODEID(-2)),VINT(6,NODEID(-2)),WINT(6,NODEID(-2)),PB2(MBA,NODEID(-2)),PT(MBA,NODEID(-2)))
  ...
  !$acc& PI,DT,UX,UY,UZ,LINKTAB,SKK,IVV,FB,AA,B,WORK,&
  !$acc& ND2,W,ND,PHI,A,AINV,PP2,UINT,VINT,WINT,PB2,PT)
  !$acc parallel loop gang num_gangs(NODEID(-2)) vector vector_length(16)
  DO INOD=1,NODEID(-2)
   ...
   DO INTI=1,6
      ...
      CALL WEIGHTF2_SHA(LNODE,NODN,NLMAX,KW,R,R0,CC,&
        NODEID,NWALLID,COORX,COORY,COORZ,INOD,&
        XINT,YINT,ZINT,NLINK(INOD)%I(0:KW2),NN,ND(:,INOD),W(:,INOD),2,&
        I,I2,KW2,RIAV,DR,DR2,WWI)
      ...
      IF(NN.LE.3)THEN
         CALL SHEPARDSF_SHA(PHI(:,INOD),NN,W(:,INOD),NLMAX,I,WWI)        
         GOTO 30
       ENDIF

       CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A(:,:,INOD),B(:,:,INOD),NN,ND(:,INOD),W(:,INOD),&
        COORX,COORY,COORZ,I,NI,J,K,PB2(:,INOD),PP2(:,:,INOD),WWI)

      CALL BASEFUN_SHA(MBA,PT(:,INOD),XINT,YINT,ZINT)

      IF(MBA.EQ.4)THEN
         CALL FINDINV4X4(A(:,:,INOD),AINV(:,:,INOD),WWI)
         IF(ABS(WWI).LT.1E-15)THEN
       ...
       CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV(:,:,INOD),B(:,:,INOD),PT(:,INOD),AA(:,:,INOD),PHI(:,INOD),I,J,K)

      30    CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND(:,INOD),NN2,ND2(:,INOD),PHI(:,INOD),1/6D0)

       !! VELOCITY AT INTEGRATION POINTS
      !$acc loop
      DO I2=1,NN
         I=ND(I2,INOD)
         UINT(INTI,INOD)=UINT(INTI,INOD)+PHI(I2,INOD)*UX(I,2)
         VINT(INTI,INOD)=VINT(INTI,INOD)+PHI(I2,INOD)*UY(I,2)
         WINT(INTI,INOD)=WINT(INTI,INOD)+PHI(I2,INOD)*UZ(I,2)
       ENDDO
   ENDDO
   ...
   CALL WEIGHTF2_SHA(LNODE,NODN,NLMAX,KW,R,R0,CC,&
      NODEID,NWALLID,COORX,COORY,COORZ,INOD,&
      XQ,YQ,ZQ,NLINK(INOD)%I(0:KW2),NN,ND(:,INOD),W(:,INOD),2,&
      I,I2,KW2,RIAV,DR,DR2,WWI)
   ...
   IF (NN.LE.0) THEN
    ...
    CALL BASEFUN_SHA(MBA,PT(:,INOD),XQ,YQ,ZQ)
    ...
    IF(MBA.EQ.4)THEN
      ...
      IF(ABS(WWI).LT.1E-15)THEN
        PRINT*,"     [ERR] SINGULAR MATRIX A, ADET ",WWI
        PRINT*,"     [---] LOC ",XQ,YQ,ZQ
        CALL SHEPARDSF_SHA(PHI(:,INOD),NN,W(:,INOD),NN,I,WWI)
        GOTO 31
      ENDIF
    ELSE
      PRINT*," [ERR] FINDINV NOT CODED FOR MBA =",MBA
    ...
    31  CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND(:,INOD),NN2,ND2(:,INOD),PHI(:,INOD),-1D0)
    
    K=0
    !$acc loop
    DO I2=1,NLINK(INOD)%I(0)
      I=NLINK(INOD)%I(I2)
      CALL SEARCH(I,J,ND2(1:NN2,INOD),NN2)
      IF((NODEID(I).LE.10).AND.(NWALLID(I,2).NE.-10).AND.(J.NE.0))THEN

        K=K+1
        LINKTAB((INOD-1)*IVV(0)+K)=I
        SKK((INOD-1)*IVV(0)+K)=WORK(J,INOD)
        WORK(J,INOD)=0D0
      ENDIF
    ENDDO
    ...
    IVV(INOD)=K+1
    CALL SEARCH(INOD,J,ND2(1:NN2,INOD),NN2)
    IF(J.NE.0)THEN
      LINKTAB((INOD-1)*IVV(0)+K+1)=INOD
      SKK((INOD-1)*IVV(0)+K+1)=WORK(J,INOD)
      WORK(J,INOD)=0D0
    ENDIF

    !! RHS
    TMPR=R0I*PI/3D0*(UINT(1,INOD)+VINT(2,INOD)-UINT(3,INOD)-VINT(4,INOD)+WINT(5,INOD)-WINT(6,INOD))
  ...
  !DO INOD=1,NODEID(-2)
      ...
      !J=NNN(INOD)
      !WRITE(19,"(300I7)"),LINKTAB((INOD-1)*IVV(0)+1:(INOD-1)*IVV(0)+IVV(INOD))
      !WRITE(9,*),SKK((INOD-1)*IVV(0)+1:(INOD-1)*IVV(0)+IVV(INOD)),FB(INOD)
   !ENDDO
...
MODULE VSS_K
   ...
   CONTAINS
   ATTRIBUTES(GLOBAL) SUBROUTINE VELOCITY_SMOOTH_SHA_K(KW,LNODE,NODN,NODEID,NWALLID,NLMAX,MBA,COORX,COORY,COORZ,R1,DDR,UXC1,UYC1,UZC1,UXC3,UYC3,UZC3)
   USE INTERPNEW
   IMPLICIT NONE
   ...
   REAL(KIND=8),MANAGED::UXC1(NODN),UYC1(NODN),UZC1(NODN)
   REAL(KIND=8),MANAGED::UXC3(NODN),UYC3(NODN),UZC3(NODN)
...
SUBROUTINE VELOCITY_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::INOD,NN,IDWEI,KW2
  INTEGER(KIND=4)::I,J,K,I2,NI
  INTEGER(KIND=4),MANAGED,ALLOCATABLE::ND(:,:)
  REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),PHI(:,:),AA(:,:,:),W(:,:),PP2(:,:,:),A(:,:,:),AINV(:,:,:),PT(:,:),PB2(:,:)
  REAL(KIND=8)::XQ,YQ,ZQ,PINT,DEW,WWI
  REAL(KIND=8)::RIAV,DR,DR2,GAMMA,GAMMA2,CCIAV,RCI,ETMP
  REAL(KIND=8),MANAGED::UXC1(NODN),UYC1(NODN),UZC1(NODN)
  REAL(KIND=8),MANAGED::UXC3(NODN),UYC3(NODN),UZC3(NODN)
  REAL(KIND=8)::UXL1,UYL1,UZL1,UXL3,UYL3,UZL3

   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)))
   ALLOCATE(W(NLMAX,NODEID(-1)),ND(0:NLMAX,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)))
   ALLOCATE(A(MBA,MBA,NODEID(-1)),AINV(MBA,MBA,NODEID(-1)),PT(MBA,NODEID(-1)),PB2(MBA,NODEID(-1)))
  ...
  !$acc data copyin(NODEID,NWALLID,KW,R,R0,CC,&
  !$acc& COORX,COORY,COORZ,NLINK,LNODE,NLMAX,NODN,MBA,IDWEI,&
  !$acc& UX,UY,UZ,UXC1,UYC1,UZC1,UXC3,UYC3,UZC3,GAMMA,GAMMA2,&
  !$acc& B,PHI,AA,W,ND,PP2,A,AINV,PB2,PT)
  !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16)
  DO INOD=1,NODEID(-1) 
   ...
   CALL WEIGHTF1_SHA(LNODE,NLMAX,NODN,KW,R,R0,CC,NODEID,NWALLID,&
      COORX,COORY,COORZ,INOD,XQ,YQ,ZQ,NLINK(INOD),&
      NN,ND(:,INOD),W(:,INOD),IDWEI,0,I,I2,KW2,RIAV,CCIAV,RCI,ETMP,DEW,DR,DR2,WWI)
   ...
   30  CONTINUE
    ...
    !$acc loop reduction(+:UXL1,UYL1,UZL1,UXL3,UYL3,UZL3)
    DO I2=1,NN
      I=ND(I2,INOD)
      UXL1=UXL1+PHI(I2,INOD)*UX(I,1)
      UYL1=UYL1+PHI(I2,INOD)*UY(I,1)
      UZL1=UZL1+PHI(I2,INOD)*UZ(I,1)
      UXL3=UXL3+PHI(I2,INOD)*UX(I,3)
      UYL3=UYL3+PHI(I2,INOD)*UY(I,3)
      UZL3=UZL3+PHI(I2,INOD)*UZ(I,3)
    ENDDO
...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::INOD,NN,IDWEI,KW2
   INTEGER(KIND=4)::I,J,K,I2,NI
   REAL(KIND=8),MANAGED,ALLOCATABLE::AA(:,:,:),PHI(:,:),A(:,:,:),AINV(:,:,:),PP2(:,:,:)
   REAL(KIND=8)::W(NLMAX),B(MBA,NLMAX)
   REAL(KIND=8)::PT(MBA)
   REAL(KIND=8)::PB2(MBA)
   ...
   ALLOCATE(AA(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)),A(MBA,MBA,NODEID(-1)),AINV(MBA,MBA,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)),ND(0:NLMAX,NODEID(-1)))
   ...
   !$acc& R,R0,CC,COORX,COORY,COORZ,NLINK,LNODE,&
   !$acc& NLMAX,NODN,MBA,IDWEI,B,AA,PHI,A,AINV,PP2,ND,DDR)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16) private(B,W,PB2,PT)
   DO INOD=1,NODEID(-1)
      ...
      CALL WEIGHTF1_SHA(LNODE,NLMAX,NODN,KW,R,R0,CC,NODEID,NWALLID,&
      COORX,COORY,COORZ,INOD,XQ,YQ,ZQ,NLINK(INOD),&
      NN,ND(:,INOD),W,IDWEI,0,I,I2,KW2,RIAV,CCIAV,RCI,ETMP,DEW,DR,DR2,WWI)
      ...
      IF (NN.LE.0) THEN
       ...
       IF(NN.LE.3)THEN
         CALL SHEPARDSF_SHA(PHI(:,INOD),NN,W,NLMAX,I,WWI)
         GOTO 30
       ENDIF

       CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A(:,:,INOD),B,NN,ND(:,INOD),W,&
       COORX,COORY,COORZ,I,NI,J,K,PB2,PP2(:,:,INOD),WWI)
      ...
      CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV(:,:,INOD),B,PT,AA(:,:,INOD),PHI(:,INOD),I,J,K)

      30  PINT=0D0
      !$acc loop reduction(+:PINT)
    DO I2=1,NN
      PINT=PINT+PHI(I2,INOD)*PTMP(ND(I2,INOD))      
    ENDDO
    P(INOD)=PINT
