...
IMPLICIT NONE
  ...
  INTEGER(KIND=4)::INOD,ND(0:NLMAX),ND2(0:NLMAX),NN,NN2,KW2
  ...
  ALLOCATE(AA(MBA,NLMAX,NODEID(-2)),B(MBA,NLMAX,NODEID(-2)),WORK(4*NLMAX,NODEID(-2)),W(NLMAX,NODEID(-2)))
  ALLOCATE(PHI(NLMAX,NODEID(-2)),A(MBA,MBA,NODEID(-2)),AINV(MBA,MBA,NODEID(-2)),PP2(MBA,MBA,NODEID(-2)))
  ...
  !$acc& W,PHI,A,AINV,PP2,UINT,VINT,WINT,PB2,PT)
  !$acc parallel loop gang num_gangs(NODEID(-2)) vector vector_length(16) private(ND,ND2)
  DO INOD=1,NODEID(-2)
   ...
   DO INTI=1,6
      ...
      CALL WEIGHTF2_SHA(LNODE,NODN,NLMAX,KW,R,R0,CC,&
        NODEID,NWALLID,COORX,COORY,COORZ,INOD,&
        XINT,YINT,ZINT,NLINK(INOD)%I(0:KW2),NN,ND,W(:,INOD),2,&
        I,I2,KW2,RIAV,DR,DR2,WWI)
      ...
      30    CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND,NN2,ND2,PHI(:,INOD),1/6D0)
   ...
   CALL WEIGHTF2_SHA(LNODE,NODN,NLMAX,KW,R,R0,CC,&
      NODEID,NWALLID,COORX,COORY,COORZ,INOD,&
      XQ,YQ,ZQ,NLINK(INOD)%I(0:KW2),NN,ND,W(:,INOD),2,&
      I,I2,KW2,RIAV,DR,DR2,WWI)
   ...
   IF (NN.LE.0) THEN
    ...
    31  CALL STIFFNESS_T1_SHA(NLMAX,4*NLMAX,WORK(:,INOD),NN,ND,NN2,ND2,PHI(:,INOD),-1D0)
