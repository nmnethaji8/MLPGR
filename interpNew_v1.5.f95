...
IMPLICIT NONE
  ...
  REAL(KIND=8)::B(MBA,NLMAX)
  REAL(KIND=8),MANAGED,ALLOCATABLE::W(:,:),PHI(:,:)
  ...
  ALLOCATE(WORK(4*NLMAX,NODEID(-2)),W(NLMAX,NODEID(-2)))
  ...
  !$acc& PI,DT,UX,UY,UZ,LINKTAB,SKK,IVV,FB,WORK,&
  !$acc& W,PHI,A,PP2,PB2,PT)
  !$acc parallel loop gang num_gangs(NODEID(-2)) vector vector_length(16) private(ND,ND2,AA,AINV,UINT,VINT,WINT,B)
  DO INOD=1,NODEID(-2)
   ...
   DO INTI=1,6
      ...
      IF(NN.LE.3)THEN
       ...
       CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A(:,:,INOD),B,NN,ND,W(:,INOD),&
      ...
      IF(MBA.EQ.4)THEN
       ...
       CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV,B,PT(:,INOD),AA,PHI(:,INOD),I,J,K)
