...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::INOD,NN,IDWEI,KW2
   INTEGER(KIND=4)::I,J,K,I2,NI
   REAL(KIND=8),MANAGED,ALLOCATABLE::PP2(:,:,:)
   REAL(KIND=8)::PHI(NLMAX),W(NLMAX),B(MBA,NLMAX)
   REAL(KIND=8)::A(MBA,MBA),AINV(MBA,MBA)
   REAL(KIND=8)::PT(MBA)
   REAL(KIND=8)::PB2(MBA)
   REAL(KIND=8)::XQ,YQ,ZQ,AA(MBA,NLMAX),PINT
   REAL(KIND=8)::RIAV,DR,DR2,CCIAV,RCI,ETMP,DEW,WWI
   ...
   ALLOCATE(PP2(MBA,MBA,NODEID(-1)),ND(0:NLMAX,NODEID(-1)))
   ...
   !$acc& NLMAX,NODN,MBA,IDWEI,PP2,ND,DDR)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16) private(AA,AINV,A,B,W,PHI,PB2,PT)
   DO INOD=1,NODEID(-1)
