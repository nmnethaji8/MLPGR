...
IMPLICIT NONE
  ...
  ALLOCATE(WORK(4*NLMAX,NODEID(-2)))
  ...
  !$acc& PT)
  !$acc parallel loop gang num_gangs(NODEID(-2)) vector vector_length(16) private(ND,ND2,AA,AINV,UINT,VINT,WINT,B,A,PP2,PB2,PHI,W)
  DO INOD=1,NODEID(-2)
   ...
   DO INTI=1,6
      ...
      CALL WEIGHTF2_SHA(LNODE,NODN,NLMAX,KW,R,R0,CC,&
        NODEID,NWALLID,COORX,COORY,COORZ,INOD,&
        XINT,YINT,ZINT,NLINK(INOD)%I(0:KW2),NN,ND,W,2,&
        I,I2,KW2,RIAV,DR,DR2,WWI)
   ...
   CALL WEIGHTF2_SHA(LNODE,NODN,NLMAX,KW,R,R0,CC,&
      NODEID,NWALLID,COORX,COORY,COORZ,INOD,&
      XQ,YQ,ZQ,NLINK(INOD)%I(0:KW2),NN,ND,W,2,&
      I,I2,KW2,RIAV,DR,DR2,WWI)
   ...
   IF (NN.LE.0) THEN
    ...
    IF(MBA.EQ.4)THEN
      ...
      IF(ABS(WWI).LT.1E-15)THEN
        PRINT*,"     [ERR] SINGULAR MATRIX A, ADET ",WWI
        PRINT*,"     [---] LOC ",XQ,YQ,ZQ
        CALL SHEPARDSF_SHA(PHI,NN,W,NN,I,WWI)
        GOTO 31
      ENDIF
    ELSE
      PRINT*," [ERR] FINDINV NOT CODED FOR MBA =",MBA
