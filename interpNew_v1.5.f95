...
IMPLICIT NONE
  ...
  REAL(KIND=8),ALLOCATABLE::B(:,:,:),W(:,:),PHI(:,:)
...
SUBROUTINE VELOCITY_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::INOD,ND(0:NLMAX),NN,IDWEI,KW2
  INTEGER(KIND=4)::I,J,K,I2,NI
  REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),PHI(:,:),AA(:,:,:),W(:,:)
  REAL(KIND=8)::A(MBA,MBA),AINV(MBA,MBA),PT(MBA)
  REAL(KIND=8)::PB2(MBA),PP2(MBA,MBA)
  ...
  ALLOCATE(NNN(NODEID(-1)))
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)),W(NLMAX,NODEID(-1)))
  NNN=0
  ...
  !$acc& UX,UY,UZ,UXC1,UYC1,UZC1,UXC3,UYC3,UZC3,GAMMA,GAMMA2,B,NNN,PHI,AA,W)
  !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(8) private(ND,A,PB2,PP2,PT,AINV)
  DO INOD=1,NODEID(-1) 
   ...
   CALL WEIGHTF1_SHA(LNODE,NLMAX,NODN,KW,R,R0,CC,NODEID,NWALLID,&
      ...
      NN,ND,W(:,INOD),IDWEI,0,I,I2,KW2,RIAV,CCIAV,RCI,ETMP,DEW,DR,DR2,WWI)
   ...
   IF (NN.LE.0) THEN
    ...
    CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A,B(:,:,INOD),NN,ND,W(:,INOD),&
      COORX,COORY,COORZ,I,NI,J,K,PB2,PP2,WWI)
      ...
      CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV,B(:,:,INOD),PT,AA(:,:,INOD),PHI(:,INOD),I,J,K)
 ...
 ENDDO
  !$acc end data
   ...
   DO INOD=1,NODEID(-1)    
      ...
      WRITE(9,*),UX(INOD,1),UY(INOD,1),UZ(INOD,1),UX(INOD,3),UY(INOD,3),UZ(INOD,3)
...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::I,J,K,I2,NI
   REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),AA(:,:,:),PHI(:,:),A(:,:,:),AINV(:,:,:),PP2(:,:,:),W(:,:)
   REAL(KIND=8)::PT(MBA)
   REAL(KIND=8)::PB2(MBA)
   ...
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)),A(MBA,MBA,NODEID(-1)),AINV(MBA,MBA,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)),W(NLMAX,NODEID(-1)),ND(0:NLMAX,NODEID(-1)))
   ...
   !$acc& NLMAX,NODN,MBA,IDWEI,B,AA,PHI,A,AINV,PP2,W,ND,DDR)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(16) private(PB2,PT)
   DO INOD=1,NODEID(-1)
      ...
      CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV(:,:,INOD),B(:,:,INOD),PT,AA(:,:,INOD),PHI(:,INOD),I,J,K)
