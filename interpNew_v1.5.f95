...
SUBROUTINE VELOCITY_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::INOD,ND(0:NLMAX),NN,IDWEI,KW2
  INTEGER(KIND=4)::I,J,K,I2,NI
  REAL(KIND=8)::PHI(NLMAX),W(NLMAX)
  REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:)
  REAL(KIND=8)::A(MBA,MBA),AINV(MBA,MBA),PT(MBA)
  REAL(KIND=8)::PB2(MBA),PP2(MBA,MBA)
  REAL(KIND=8)::XQ,YQ,ZQ,AA(MBA,NLMAX),PINT,DEW,WWI
  REAL(KIND=8)::RIAV,DR,DR2,GAMMA,GAMMA2,CCIAV,RCI,ETMP
  ...
  INTEGER(KIND=4),MANAGED,ALLOCATABLE::NNN(:)
  REAL(KIND=8),MANAGED,ALLOCATABLE::PHII(:,:)

  ALLOCATE(NNN(NODEID(-1)),PHII(NLMAX,NODEID(-1)))
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)))
  NNN=0
  ...
  !$acc& UX,UY,UZ,UXC1,UYC1,UZC1,UXC3,UYC3,UZC3,GAMMA,GAMMA2,B,NNN,PHII)
  !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(4) private(ND,W,PHI,A,PB2,PP2,PT,AINV,AA)
  DO INOD=1,NODEID(-1) 
   ...
   IF (NN.LE.0) THEN
    ...
    CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A,B(:,:,INOD),NN,ND,W,&
      ...
      CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV,B(:,:,INOD),PT,AA,PHI,I,J,K)
