...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   REAL(KIND=8),ALLOCATABLE::PHII(:,:)

   ALLOCATE(NNN(NODEID(-1)),PHII(NLMAX,NODEID(-1)))
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)))
   NNN=0
   PHII=0D0
   ...
   !$acc& R,R0,CC,COORX,COORY,COORZ,NLINK,LNODE,NLMAX,NODN,MBA,IDWEI,B,AA)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(8) private(ND,&
   !$acc& W,PHI,A,PB2,PP2,PT,AINV)
   DO INOD=1,NODEID(-1)
      ...
      CALL SHAPEFUN_PHI_SHA(MBA,NLMAX,NN,AINV,B(:,:,INOD),PT,AA(:,:,INOD),PHI,I,J,K)
