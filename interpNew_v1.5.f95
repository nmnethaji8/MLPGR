...
SUBROUTINE PRESSURE_SMOOTH_SHA(LNODE,NODN,NODEID,NWALLID,&
   ...
   INTEGER(KIND=4)::I,J,K,I2,NI
   REAL(KIND=8)::W(NLMAX)
   REAL(KIND=8),MANAGED,ALLOCATABLE::B(:,:,:),AA(:,:,:),PHI(:,:),A(:,:,:),AINV(:,:,:),PP2(:,:,:)
   REAL(KIND=8)::PT(MBA)
   REAL(KIND=8)::PB2(MBA)
   ...
   ALLOCATE(B(MBA,NLMAX,NODEID(-1)),AA(MBA,NLMAX,NODEID(-1)),PHI(NLMAX,NODEID(-1)),A(MBA,MBA,NODEID(-1)),AINV(MBA,MBA,NODEID(-1)),PP2(MBA,MBA,NODEID(-1)))
   NNN=0
   ...
   !$acc& R,R0,CC,COORX,COORY,COORZ,NLINK,LNODE,NLMAX,NODN,MBA,IDWEI,B,AA,PHI,A,AINV,PP2)
   !$acc parallel loop gang num_gangs(NODEID(-1)) vector vector_length(8) private(ND,&
   !$acc& W,PB2,PT)
   DO INOD=1,NODEID(-1)
      ...
      IF (NN.LE.0) THEN
       ...
       CALL SHAPPARA_R_SHA(LNODE,MBA,NLMAX,A(:,:,INOD),B(:,:,INOD),NN,ND,W,&
