PROGRAM THREED_BREAKINGWAVE
   ...
   TYPE(NODELINKTYP),MANAGED,ALLOCATABLE::MLDOM,FSDOM
   ...
   OPEN(UNIT=9, FILE=" /home/vsriram/MATLAB-Drive/output.txt")
   ...
   DO II=1,BNDNP
      IJ=NODEID(-2)+II
      BNDXY(II,1)=COORX(IJ,1)
      BNDXY(II,2)=COORY(IJ,1)
      BNDXY(II,3)=COORZ(IJ,1)

      ! Assuming  bottom is at 0d0
      IF(ABS(COORZ(IJ,1)-0D0).LT.TMPR7)THEN
         ...
         IF(NODEID(IJ).EQ.1)THEN
           BNDFIX(2,0)=BNDFIX(2,0)+1
           BNDFIX(2,BNDFIX(2,0))=IJ
         ENDIF

         IF(NODEID(IJ).EQ.7)THEN
           BNDFIX(3,0)=BNDFIX(3,0)+1
           BNDFIX(3,BNDFIX(3,0))=IJ
         ENDIF
      ENDIF
...
501   CONTINUE
      ...
      IF(I_WM.EQ.15)THEN
         ...
         DO NNI=NODEID(-2)+1,NODEID(-3)
            ...
            PRESS_DR(NNI2) = FP%MLVAL(NNI2,1) &
                              + (1000D0*9.81D0*(COORZ(NNI,2)-H0))

            !Assuming bottom is at Z=0
            IF( ABS(COORZ(NNI,1) - 0D0) .LT. 0.00001D0)THEN 
               COORZ(NNI,2)=0D0
               UZ(NNI,1:2)=0D0
               TANK_A(NNI,3)=0D0          
            ENDIF
      ...
      CALL VELOCITY_SMOOTH_SHA(LNODE,NPOI1,NODEID(-2:NPOI1),NWALLID, &
         COORX(:,2),COORY(:,2),COORZ(:,2),50,MBAS,KW,R1,DDR)
      ...
      CALL NEWCOOR4(BNDNP,BNDXY,BNDFIX,FSNOD1,FSNOD2,DDR, &
         DOMX,DOMY,DOMZ)
